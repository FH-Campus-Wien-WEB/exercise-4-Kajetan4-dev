...
app.get("/logout", function (req, res) {
  // Destroy the session container completely
  req.session.destroy(function (err) {
    ...
    res.clearCookie("connect.sid"); // Clear the session cookie matching express-session defaults
      ...
      .then(apiRes => {
        ...
        return apiRes.text().then(data => {
          ...
          try {
            ...
            console.error('Failed to parse OMDb movie response:', parseError);
          ...
          if (response.Response === 'True') {
            ...
            const stringToArray = (str) => {
              return (str && str !== "N/A") ? str.split(",").map(item => item.trim()) : [];
            };

            const movieData = {
              imdbID: response.imdbID,
              Title: response.Title,
              // OMDb returns "16 Dec 2003", we convert to "2003-12-16" to match your DB
              Released: response.Released !== "N/A" ? new Date(response.Released).toISOString().split('T')[0] : "",
              // OMDb returns "201 min", parseInt safely grabs just the 201
              Runtime: parseInt(response.Runtime) || null,
              Genres: stringToArray(response.Genre),
              Directors: stringToArray(response.Director),
              Writers: stringToArray(response.Writer),
              Actors: stringToArray(response.Actors),
            ...
            // Save the newly formatted movie to the user's collection
            movieModel.setUserMovie(username, imdbID, movieData);
            res.sendStatus(201); // 201 Created for successful insertion
          } else {
            console.error('OMDb error response:', response.Error);
            res.sendStatus(404); // Movie not found on OMDb
          }
        });
      })
      .catch((err) => {
        ...
        if (err.name === 'AbortError') {
          console.error('OMDb API movie details request timeout');
          return res.sendStatus(504); // Gateway Timeout
        }
        console.error('OMDb API movie details error:', err);
